...
// TODO: Check rate limiting to prevent spam
// Example:
// $db = new PDO('mysql:host=localhost;dbname=viddia', 'username', 'password');
// $stmt = $db->prepare('SELECT COUNT(*) as count FROM verification_tokens WHERE email = ? AND created_at > DATE_SUB(NOW(), INTERVAL 5 MINUTE)');
// $stmt->execute([$email]);
// $result = $stmt->fetch();
// if ($result['count'] >= 3) {
...
// $stmt = $db->prepare('UPDATE verification_tokens SET used_at = NOW() WHERE email = ? AND used_at IS NULL');
// $stmt->execute([$email]);
...
error_log("New verification token for $email: $token (expires: $expiresAt)");
...
<body>
    <div class='container'>
        ...
        <div class='content'>
            ...
            <p>Para completar seu cadastro, clique no botão abaixo para verificar seu e-mail:</p>
